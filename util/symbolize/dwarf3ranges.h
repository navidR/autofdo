...
// This struct does contain header, and each rng specific related data.
struct RngListsData {
  RngListsData()
  : unit_length(0),
    version(0),
...
class AddressRangeList {
 public:
  ...
  const char* GetRngListsElementAddressByIndex(uint64 addr_base, uint64 rng_index);
 ...
 private:
  ...
  void ReadDwarfRngLists(uint64 base, RangeList* output, const char* pos, uint64 addr_base, RngListsData& rnglistsdata);
  ...
  // then addr_base will be the begining of the data address (right after header).
