...
const char* CompilationUnit::ProcessAttribute(
  ...
  switch (form) {
    ...
    case DW_FORM_rnglistx: {
      ...
      uint64 address_ = (uint64) inline_stack_handler->GetRngListsElementAddressByIndex(ranges_base_, rng_index);
...
void CompilationUnit::ProcessAttributeUnsigned(uint64 offset,
    ...
    if (attr == DW_AT_GNU_dwo_id) // In DWARF 5, this attribute has moved to header field.
      ...
      InlineStackHandler* inline_stack_handler = dynamic_cast<InlineStackHandler*>(handler_);
      if (inline_stack_handler) {
        inline_stack_handler->SetRngListsSectionBaseAddress(ranges_base_);
      } 
      else {
        LOG(WARNING) << "It is not possible to set .debug_addr base address. This means, you might run into trouble yo use .debug_rnglists to calculate the ranges.";
      }      
